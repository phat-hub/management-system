<?php $this->layout("layouts/default", ["title" => "Thêm phân công giảng dạy"]) ?>
<?php $this->start("page") ?>

<div class="container py-5">
  <div class="bg-white p-5 rounded-3 shadow-sm">
    <h2 class="fw-bold mb-4">Thêm phân công giảng dạy</h2>

    <form action="/teaching_assignment/create" method="POST">
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <select class="form-select <?= isset($errors['subject_id']) ? 'is-invalid' : '' ?>" name="subject_id">
            <option value="" disabled <?= empty($old['subject_id']) ? 'selected' : '' ?>>-- Chọn học phần --</option>
            <?php foreach ($subjects as $subject): ?>
              <option value="<?= $subject->id ?>" <?= (isset($old['subject_id']) && $old['subject_id'] == $subject->id) ? 'selected' : '' ?>>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select <?= isset($errors['day_of_week']) ? 'is-invalid' : '' ?>" name="day_of_week">
            <option value="" disabled <?= empty($old['day_of_week']) ? 'selected' : '' ?>>-- Chọn thứ --</option>
            <?php 
              $days = ['Monday' => 'Thứ 2', 'Tuesday' => 'Thứ 3', 'Wednesday' => 'Thứ 4', 'Thursday' => 'Thứ 5', 'Friday' => 'Thứ 6', 'Saturday' => 'Thứ 7'];
              foreach ($days as $value => $label): ?>
                <option value="<?= $value ?>" <?= (isset($old['day_of_week']) && $old['day_of_week'] == $value) ? 'selected' : '' ?>>
                  <?= $label ?>
                </option>
            <?php endforeach; ?>
          </select>
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="number" name="start_period" class="form-control <?= isset($errors['start_period']) ? 'is-invalid' : '' ?>"
                 min="1" max="9" value="<?= $old['start_period'] ?? '' ?>">
        ...
        <div class="col-md-6">
          ...
          <input type="number" name="end_period" class="form-control <?= isset($errors['end_period']) ? 'is-invalid' : '' ?>"
                 min="1" max="9" value="<?= $old['end_period'] ?? '' ?>">
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <select class="form-select <?= isset($errors['classroom_id']) ? 'is-invalid' : '' ?>" name="classroom_id">
            <option value="" disabled <?= empty($old['classroom_id']) ? 'selected' : '' ?>>-- Chọn phòng học --</option>
            <?php foreach ($classrooms as $room): ?>
              <option value="<?= $room->id ?>" <?= (isset($old['classroom_id']) && $old['classroom_id'] == $room->id) ? 'selected' : '' ?>>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select <?= isset($errors['user_id']) ? 'is-invalid' : '' ?>" name="user_id">
            <option value="" disabled <?= empty($old['user_id']) ? 'selected' : '' ?>>-- Chọn giảng viên --</option>
            <?php foreach ($users as $teacher): ?>
              <option value="<?= $teacher->id ?>" <?= (isset($old['user_id']) && $old['user_id'] == $teacher->id) ? 'selected' : '' ?>>
                <?= $this->e($teacher->name) ?>
              </option>
            <?php endforeach; ?>
          </select>
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" value="<?= $latestSemester->semester ?> - <?= $latestSemester->academicYear ?>" readonly>
        <input type="hidden" name="semester_id" value="<?= $latestSemester->id ?>">
      </div>
